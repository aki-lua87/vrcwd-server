...
namespace aki.Lua87
{
    ...
    {
        private readonly string logPrefix = $"[aki_lua87] {typeof(GetHistory)}: ";
        [SerializeField] private GameObject spornPortalGameObjectPrefab;
        ...
        void Start()
        {
            nextPortalPositionX = 0f;
            nextPortalPositionZ = 0f;
        ...
        private void CreatePortal(string worldID, string worldName)
        {
            ...
            var portal = Instantiate(spornPortalGameObjectPrefab, Vector3.zero, Quaternion.identity, this.transform);
            portal.GetComponent<SpornPortal>().worldid = worldID;
            portal.transform.localPosition = new Vector3(nextPortalPositionX, 0, nextPortalPositionZ);
